...
> [!TIP]
>
> 吐槽：如果希望快速使用本末叭并且跳过复杂的环节，就可以直接参考本文的 `项目初始、项目依赖` 后，直接前往复制代码仓库中的 `./src/app/` 目录覆盖本地（可以完全覆盖），并且把 `./` 下的一些重要文件（而不是目录）保持大体的一致（有少部分内容需要修改）即可。
...
```shell
$ mv ./.template.gitignore ./.gitignore && cat ./.gitignore
# ./.gitignore: 忽略模板
...
如果上述运用成熟，可以封装为一个脚手架，以供快速配置而不是多次复制粘贴。
